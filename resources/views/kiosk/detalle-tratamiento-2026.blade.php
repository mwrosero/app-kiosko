...
<script>
	...
	document.addEventListener("DOMContentLoaded", async function () {
		...
		$('body').on('change', '.atencionInmediata-input', function() {
	    ...
	    $(document).on('click', '.btn-agendar', async function(){
	        ...
	        if(esTerapiaAgrupada){
	            ...
	            dataCita.datosTratamiento = detalleTratamiento;
	...
	async function obtenerDataRelSeleccionados() {
        let seleccionados = $(".atencionInmediata-input:checked").map(function() {
            return JSON.parse($(this).attr("prestacion-rel")); // Obtiene el atributo data-rel como objeto
	...
	async function mostrarDetalleOrdenModal(detalle){
		$('.infoAgendaMultiple').addClass('d-none');
		$('#all-checkbox').removeClass('d-none');
		let elemContent = ``;
		...
		}else if(detalle.obtenerValoresOrden !== null && detallePrestacionesValores.code == 200 && detalle.esPagada == "N"){
			console.log("DETALLES DE TERAPIAS AGRUPADAS")
			console.log(detallePrestacionesValores)
                        ...
                        if(datosServicio.esAgendable == "N"){
                            ...
                            if(datosServicio.permiteReserva == 'S'){
                                if (datosServicio.habilitaBotonAgendar == 'S' && datosServicio.esExterna == "N") {
                                    ...
                                    respuestaAgenda += `<div qty-rel='${qtyMaximaAgrupado}' esTerapiAgrupada-rel='${esTerapiaAgrupada}' url-rel="${ruta}" data-rel='${JSON.stringify(datosServicio)}' class="btn p-3 bg-royal-blue text-white rounded-12 fs-18 line-height-24 w-50 btn-agendar ${classDisabled}">Agendar</div>`;
                                } else {
                                    if(datosServicio.esExterna == "N"){
